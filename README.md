...
- ✅ **Phase 4 — 관리자 도구**: 관리자 대시보드(통계), 회원/목회자 승인·역할 관리, 콘텐츠 CRUD(슬라이드·영상·PDF, 게시 토글), AI 슬라이드 생성기, 묵상 승인 큐(AI 초안), 공지사항(한국어 작성→AI 4개 언어 번역→게시). 관리자 UI는 한국어
- ✅ **Phase 5 — 성도 기능 + 마감**: 오늘의 묵상(게시 묵상 표시), 성경 Q&A 챗봇(목회 상담 유도), 공지사항 노출, 다크 모드 토글, SEO(hreflang·메타데이터·sitemap·robots·favicon), Vercel 배포 설정(`vercel.json`), 관리자 매뉴얼
...
관리자(`role='admin'`)로 로그인하면 상단 헤더/하단 탭에 **관리자** 메뉴가 나타나고 `/admin`에 접근할 수 있습니다.

### 회원 승인
1. `관리자 → 회원` 탭
2. **목회자로 가입한 회원**은 "승인 대기" 상태로 표시됩니다. **승인** 버튼을 누르면 설교 작성기 전체 기능이 열립니다.
3. 역할 드롭다운으로 성도/목회자/관리자 권한을 변경할 수 있습니다.

### 콘텐츠 업로드
1. `관리자 → 콘텐츠 → + 새 콘텐츠`
2. 유형 선택: **슬라이드**(플랫폼 내 뷰어), **영상**(YouTube URL), **PDF**(파일 URL)
3. 제목·설명·성경·언어·대상을 입력하고, 슬라이드는 장별로 제목/본문/노트를 작성합니다.
4. **게시** 체크 후 저장하면 라이브러리에 공개됩니다. 목록에서 게시/비공개를 토글할 수 있습니다.

### AI 슬라이드 생성
1. `관리자 → AI 슬라이드`
2. 성경 구절 + 주제 + 매수 + 언어 입력 → **슬라이드 생성**
3. 생성된 슬라이드를 검토·수정한 뒤 저장하면 콘텐츠로 등록됩니다.

### 묵상 승인
1. `관리자 → 묵상`
2. 직접 작성하거나 **AI 초안** 버튼으로 생성 → **게시**로 상태를 변경하면 성도용 "오늘의 묵상"에 노출됩니다.
3. 날짜를 오늘 또는 미래로 지정해 미리 준비할 수 있습니다(해당 날짜부터 노출).

### 공지사항
1. `관리자 → 공지`
2. 한국어로 작성 후 **AI 번역**(영·프·스·독) → 검토·수정 → **게시**
3. 게시하면 각 언어 랜딩 페이지 상단에 노출됩니다.
...
- 성도 설교 요약 미리보기: 1일 3회
- 성경 Q&A: 1일 30회
- 관리자 AI 도구(슬라이드/묵상 초안/번역)는 별도 제한 없음
